...
class Controller():
    def __init__(self, view):
        ...
        for i in range(0,self.path_finder_max):
            ...
            self.path_finder[i].set_current_point(randint(10,500),randint(10,800))
    ...
    def load_calibration(self):
        self.view.set_enabled_selection(start=False,audio_out_chn=False,theme=False,new_cal=False,load_cal=False,start_default=False)
        ...
        try:
            ...
            # self.timer.stop()
            self.path_timer = QtCore.QTimer()
            self.feat,self.stone_feat,self.camera.frame = ds.getFeatures(self.camera.frame,self.contour_config_v)
            self.view.setListInfo('Calibration Loaded')
        except Exception, e:
            self.path_timer = QtCore.QTimer()
            self.feat,self.stone_feat,self.view.im = ds.getFeatures(self.view.im,self.contour_config_v)
            # cv2.imshow("coor",self.im)
    ...
    def threshold_slider2(self):
        ...
        self.path_timer.stop()
        for i in range(0,self.view.detector_amount.value()):
            self.path_finder[i].speed = 1./(float(self.view.slider2.value())/100.*float(self.view.fps))
        self.path_timer.start(self.path_finder[0].speed)
